...
| 项目 | 推荐值 | 说明 |
| --- | --- | --- |
| OBS 浏览器源 宽 × 高 | `800 × 1200` | 画布尺寸；气泡宽度、名牌宽度都按这个宽度排版（名牌 `240px` 需要画布宽度 ≳ 445px 才能用满，更窄时会按比例收窄） |

平台「基准字号」不用特意设置：它影响的是平台自身的其他部件，气泡不受影响——**正文**（`--message-font-size: 30px`）和**名字**（`--nameplate-font-size: 22px`）的字号都已固定为 `px`，与平台字号各自独立。

> 应用本样式前，请先清空「样式 → 高级 CSS 编辑器」里原有的自定义 CSS，并去掉 OBS 浏览器源里遗留的旧样式，避免旧规则与本样式互相覆盖。
