...
| 项目 | 推荐值 | 说明 |
| --- | --- | --- |
| 平台基准字号 | `30` | 只影响模版未覆盖的事件（醒目留言、礼物、上舰等）。弹幕正文已固定 `30px`（`--message-font-size`）、名字固定 `22px`（`--nameplate-font-size`），调这个值不会改变弹幕样式 |
...
| `--message-font-size` | `30px` | 弹幕正文字号（固定 `px`）。不跟随平台基准字号，改平台字号时气泡内文字大小不变 |
...
| 版本 | 变更 |
| --- | --- |
| 1.1.4 | 新增 `--message-font-size`（默认 `30px`），弹幕正文字号固定不再跟随平台基准字号；至此正文、名字、头像、气泡留白全部与平台基准字号解耦 |
